...
func TestInstallGitHook_WorktreeInstallsInCommonHooks(t *testing.T) {
	...
	count, err := InstallGitHook(true, false, false)
	if err != nil {
...
func TestInstallGitHook_Idempotent(t *testing.T) {
	...
	firstCount, err := InstallGitHook(true, false, false)
	if err != nil {
	...
	secondCount, err := InstallGitHook(true, false, false)
	if err != nil {
...
func TestInstallGitHook_LocalDevCommandPrefix(t *testing.T) {
	...
	count, err := InstallGitHook(true, true, false)
	if err != nil {
...
func TestInstallGitHook_AbsoluteGitHookPath(t *testing.T) {
	...
	count, err := InstallGitHook(true, false, true)
	if err != nil {
...
func TestRemoveGitHook_CoreHooksPathRelative(t *testing.T) {
	...
	installCount, err := InstallGitHook(true, false, false)
	if err != nil {
		t.Fatalf("InstallGitHook() error = %v", err)
	}
	if installCount == 0 {
...
func TestInstallGitHook_DoesNotOverwriteExistingBackup(t *testing.T) {
	...
	_, err := InstallGitHook(true, false, false)
	if err != nil {
		t.Fatalf("InstallGitHook() error = %v", err)
	}
