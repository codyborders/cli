...
modifying your active branch.`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			if agentName != "" {
				...
				return setupAgentHooksNonInteractive(cmd.OutOrStdout(), ag, opts)
			}
			...
			return runEnableInteractive(cmd.OutOrStdout(), agents, opts)
		},
	}
...
func runEnableInteractive(w io.Writer, agents []agent.Agent, opts EnableOptions) error {
	...
	for _, ag := range agents {
		if _, err := setupAgentHooks(ag, opts.LocalDev, opts.ForceHooks); err != nil {
...
func setupAgentHooksNonInteractive(w io.Writer, ag agent.Agent, opts EnableOptions) error {
	agentName := ag.Name()
	...
	installedHooks, err := hookAgent.InstallHooks(opts.LocalDev, opts.ForceHooks)
	if err != nil {
...
func setupGitHook() error {
	...
	if _, err := strategy.InstallGitHook(false, localDev, absoluteHookPath); err != nil {
		return fmt.Errorf("failed to install git hook: %w", err)
	}
	strategy.CheckAndWarnHookManagers(os.Stderr, localDev, absoluteHookPath)
	return nil
}
